<?php
	...
	{$nom_abs=R::find('userab','login=?',array($what->email));
		...
		foreach ($nom_abs as $nom_ab) 
		{
			...
			echo '<br>Заборгованість становить:  '.-$abonent->suma.' грн <br> ';
			if (!empty($abonent->ppp)) 
			{echo 'Останній отриманий показник:  ';}	 
			if(!empty($abonent->lx1))
			{echo 'Лічільник1:  '.$abonent->lx1.' м3<br>';}
			if(!empty($abonent->lx2))
			{echo 'Лічільник2:  '.$abonent->lx2.' м3<br>';}
			if(!empty($abonent->lt1))
			{echo 'Лічільник3:  '.$abonent->lt1.' м3<br>';}
			if(!empty($abonent->lt2))
			{echo 'Лічільник4:  '.$abonent->lt2.' м3<br>';}
...
if (isset($_SESSION['logged_user'])): 	?> 
	...
	{echo " ";
		...
		<button type='submit' name='do_add_nom_ab'> Додати інший особовий номер абонента</button> 
	...
	if (isset($data['do_pok'])=='true')	
		{
			...
			if (isset($data['do_pok'])=='true')	
			{
				...
				$kstkubiv=intval($data['lx1'])-intval($abonent->lx1)+
						intval($data['lx2'])-intval($abonent->lx2)+
						intval($data['lt1'])-intval($abonent->lt1)+	
						intval($data['lt2'])-intval($abonent->lt2);
				if ($kstkubiv>200)
				{					
			...
			'login'=>$_SESSION['logged_user']->email
			 ...
			 $pok->ipadr=$_SERVER['REMOTE_ADDR'];					 
				 R::store($pok);
				echo "Записано. Спожито станом на кінець місяця  ".date('Y рік  m місяць ',$pok->pok_date)."  ".$kstkubiv.' м3 води'; 
			}
			else
			{
		...
		if (isset($data['do_inp'])) 
			...
			if (!empty($errors))
				...
				{	
					$abonent=R::findOne('abonent','nom_ab=?',array(intval($data['nom_ab_pok'])));
					if (isset($abonent))
					{	echo '<strong>Попередній показник (установа)</strong>'.'';
						if (!empty($abonent->lx1)) 
						{echo '<br>Лічільник1:  '.$abonent->lx1.' м3 ';}
						if (!empty($abonent->lx2)) 
						{echo '<br>Лічільник2:  '.$abonent->lx2.' м3 ';}
						if (!empty($abonent->lt1)) 
						{echo '<br>Лічільник3:  '.$abonent->lt1.' м3 ';}
						if (!empty($abonent->lt2)) 
						{echo '<br>Лічільник4:  '.$abonent->lt2.' м3 ';}
						...
						echo '<br><strong>Історія показів</strong>'.'<br>';
						foreach ($poks as $pok1)
						{   
							echo 'Дата подання :'.date('Y/m/d/ H:i:s',$pok1->pok_date).'  ';
							echo 'Період :'.$pok1->period.'  ';
							if (!empty($pok1->lx1))
							{echo 'Лічільник1 : '.$pok1->lx1.'  м3 ';}
							if (!empty($pok1->lx2))
							{echo 'Лічільник2:  '.$pok1->lx2.'  м3 ';}	
							if (!empty($pok1->lt1))
							{echo 'Лічільник3:  '.$pok1->lt1.'  м3 ';}
							if (!empty($pok1->lt2)) 
							{echo 'Лічільник4:  '.$pok1->lt2.'  м3 ';}									
						}
						...
						if ($abonent->ppp==1) 
						{
							echo"<br><button type='submit' name='do_pok'>Записати показники ".$lx1."</button> 
					    </form>";
						}		
					}		
				}
		...
		if (isset($data['do_act_email'])&&($email->activ==0)) 
		{
			...
			Щоб актививувати Ваш кабінет, перейдіть по зсилці:\n
			https://zlvoda.com.ua//activation.php?login=".$_SESSION['logged_user']->email."&act=".$activation."\n\n
			З повагою, Адміністрація сайту https://zlvoda.com.ua";
			mail($_SESSION['logged_user']->email, $subject, $message, $headers, "Content-type:text/plain; Charset=utf-8\r\n") ;
...
<?php  else:	?>
	...
	<main>
		<div class="bg-grey">
			...
			<h4> Персональний кабінет користувача</h4>
			<hr>
