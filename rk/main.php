...
//ini_set('display_errors',1);
	...
	function shownopl($what,$par1=-10){
		//require "../rk/db.php";
		$nom_abs=R::find('opl','nom_ab=?',array($what));
		if (empty($what)) {
			echo "<a href='../rk/main.php'>Повернутись</a>";
		}
	    ...
	    echo '</tr>';
		$i=1;	
		$nom_abs = array_slice($nom_abs, $par1, 10);
		...
		$g=1;		
		$nom_abs=R::find('opl','nom_ab=?',array($what));
		$counter_array=ceil(count($nom_abs))/10;
		for($ii=0;$ii<$counter_array;$ii++){
			$k=$g*10;
				
				echo "
				<div class='navbtn'>
				<form action='../rk/main.php' method='POST'>
...
if (isset($_SESSION['logged_user'])): 	?> 
	...
	{echo " ";
		echo "<form action='../rk/main.php' method='POST'>
	...
	if (isset($data['do_pok'])=='true')	
		...
		{   
			...
			<button type='submit' name='do_inp'>ОК</button>
		 	</form>
		 	<?		
		}		
